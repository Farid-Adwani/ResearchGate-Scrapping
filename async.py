...
def scrape_profile_data(profile_link: str):
    with sync_playwright() as p:
        ...
        if selector.css(('.coauthor__list')):
            ...
            try:
                page.wait_for_selector(co_selector, timeout=6000)
            except TimeoutError:
        ...
        if selector.css(('a[data-lite*="cited"]')):
            ...
            try:
                page.wait_for_selector('.nova-legacy-c-modal__body-content .network-cited-modal__container a.nova-legacy-e-link', timeout=6000)
            except TimeoutError:
...
def scrape_profile_data_log(link):
    start=time.time()
    print("Scrapping link : ",link)
    try:
        result=scrape_profile_data(link)
        if result:
            print("------------------------------------------------- Profile ",link.split('/')[4]," is done ------------------------------------")
    except Exception as e:
        print("")
        print(".................................................................")
        print("*********************** ERROOOOOR in link",link.split('/')[4]," RETRYING ...")
...
def scrape_profiles_in_parallel(links):
    with Pool(processes=3) as pool:  # Adjust the number of processes as per your system's capabilities
        pool.map(scrape_profile_data_log, links)
...
while next:
    ...
    print("Scraping", len(links), "links from page", page, "====> ")
    scrape_profiles_in_parallel(links)
